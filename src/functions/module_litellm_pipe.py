    ...
    async def chat_completion(
        ...
        if cost_tracker_module_name not in sys.modules:
            try:
                ...
                raise Exception(
                    f"Module {cost_tracker_module_name} is not loaded and could not be imported."
                )

        if cost_tracker_module_name not in sys.modules:
            raise Exception(
                f"Module {cost_tracker_module_name} is still not loaded after import attempt."
            )
        ...
        for key, value in self.litellm_settings.items():
            if key not in payload:
                ...
                if self.debug:
                    print(
                        f"{self.debug_logging_prefix} Added to LiteLLM call from LITELLM_SETTINGS: {key}={value}"
                    )
        ...
        try:
            ...
            if body.get("stream", False):
                if self.debug:
                    print(
                        f"{self.debug_logging_prefix} LiteLLM returning streaming response"
                    )
                ...
                async def stream_generator():
                    ...
                    try:
                        if generate_thinking_block_flag:
                            if self.debug:
                                print(
                                    f"{self.debug_logging_prefix} Starting LiteLLM async streaming with synthetic thinking block heartbeat...."
                                )
                            ...
                            think_content_json = {
                                "choices": [
                                    {
                                        "index": 0,
                                        "delta": {"content": initial_think_msg},
                                    }
                                ]
                            }
                            ...
                            acompletion_task = asyncio.create_task(
                            ...
                            while not acompletion_task.done():
                                ...
                                if not first_chunk_received:
                                    if self.debug:
                                        print(
                                            f"{self.debug_logging_prefix} Yielding synthetic still-thinking heartbeat..."
                                        )
                            ...
                            if self.debug:
                                print(
                                    f"{self.debug_logging_prefix} Starting LiteLLM streaming request without synthetic thinking block..."
                                )
                        ...
                        if self.debug:
                            print(
                                f"{self.debug_logging_prefix} Starting to consume LiteLLM stream response..."
                            )
                        ...
                        async for chunk in response_stream:
                            ...
                            if self.debug:
                                print(
                                    f"{self.debug_logging_prefix} Streaming Chunk: {chunk}"
                                )
                            ...
                            if (
                                ...
                                final_usage_stats = (
                                    ...
                                    else chunk.usage.model_dump()
                                )
                                if self.debug:
                                    print(
                                        f"{self.debug_logging_prefix} Usage-only chunk: input={final_usage_stats.get('prompt_tokens', 'N/A')}, output={final_usage_stats.get('completion_tokens', 'N/A')}"
                                    )
                                continue
                            ...
                            for choice in chunk.choices:
                                choice_delta = (
                                    choice.delta if hasattr(choice, "delta") else None
                                )
                                if (
                                    choice_delta
                                    and hasattr(
                                        choice_delta, "provider_specific_fields"
                                    )
                                    and choice_delta.provider_specific_fields
                                ):
                                    # Handle both 'citations' (array) and 'citation' (single object) formats
                                    citation_data = (
                                        choice_delta.provider_specific_fields.get(
                                            "citations"
                                        )
                                    )
                                    ...
                                    if citation_data and isinstance(
                                        citation_data, list
                                    ):
                                        for citation in citation_data:
                                            if isinstance(citation, dict):
                                                ...
                                                if (
                                                    url
                                                    and url not in citation_urls_seen
                                                ):
                                                    ...
                                                    if self.debug:
                                                        print(
                                                            f"{self.debug_logging_prefix} Citation: {title[:50]}{'...' if len(title) > 50 else ''} - {url}"
                                                        )
                            ...
                            effective_thinking_text = (
                                reasoning_content_found or thinking_blocks_text_found
                            )

                            if effective_thinking_text:
                                if self.debug:
                                    print(
                                        f"{self.debug_logging_prefix} Reasoning content: {len(effective_thinking_text)} chars"
                                    )
                                    ...
                                    "choices": [
                                        {
                                            "index": 0,
                                            "delta": {
                                                "content": effective_thinking_text
                                            },
                                        }
                                    ]
                                }
                                    ...
                                    if (
                                        ...
                                        try:
                                            ...
                                            if query_match:
                                                ...
                                                if (
                                                    search_query.strip()
                                                ):  # Only report if we have a meaningful query
                                                    ...
                                                    if self.debug:
                                                        print(
                                                            f"{self.debug_logging_prefix} Reported web search: {search_query}"
                                                        )
                                        except Exception as e:
                                            if self.debug:
                                                print(
                                                    f"{self.debug_logging_prefix} Error parsing tool call: {e}"
                                                )
                            ...
                            if actual_content_found:
                                ...
                                if is_thinking_block_active:
                                    ...
                                    if self.debug:
                                        print(
                                            f"{self.debug_logging_prefix} Closed thinking block due to actual content"
                                        )
                                ...
                                if pending_citations:
                                    ...
                                    if self.debug:
                                        print(
                                            f"{self.debug_logging_prefix} Added citations to content: {citation_text}"
                                        )
                            ...
                            if current_time - last_update_time >= 2:
                                current_gen_tok = cost_tracking_manager.count_tokens(
                                    streamed_content_buffer
                                )
                                current_rea_tok = cost_tracking_manager.count_tokens(
                                    thinking_content_buffer
                                )
                            ...
                            if hasattr(chunk, "usage") and chunk.usage:
                                ...
                                if self.debug:
                                    print(
                                        f"{self.debug_logging_prefix} Usage in chunk: input={final_usage_stats.get('prompt_tokens', 'N/A')}, output={final_usage_stats.get('completion_tokens', 'N/A')}"
                                    )
                        ...
                        if pending_citations:
                            ...
                            final_citation_json = {
                                "choices": [
                                    {"index": 0, "delta": {"content": citation_text}}
                                ]
                            }
                            ...
                            if self.debug:
                                print(
                                    f"{self.debug_logging_prefix} Added {len(pending_citations)} remaining citations"
                                )
                        ...
                        if self.debug:
                            print(
                                f"{self.debug_logging_prefix} LiteLLM stream_generator aborted by client."
                            )
                        ...
                        if (
                            "AuthenticationError" in user_friendly_error
                            or "authentication" in user_friendly_error.lower()
                        ):
                            user_friendly_error = (
                            ...
                            "RateLimitError" in user_friendly_error
                            or "rate limit" in user_friendly_error.lower()
                        ):
                            user_friendly_error = (
                            ...
                            "APIConnectionError" in user_friendly_error
                            or "connection" in user_friendly_error.lower()
                        ):
                        ...
                        if self.debug:
                            print(
                                f"{self.debug_logging_prefix} Entering LiteLLM stream finally. Completed: {stream_completed_successfully}"
                            )

                        # Process any remaining buffer content for token counting
                        final_gen_tok = cost_tracking_manager.count_tokens(
                            streamed_content_buffer
                        )
                        final_rea_tok = cost_tracking_manager.count_tokens(
                            thinking_content_buffer
                        )
                        ...
                        if final_usage_stats:
                            if self.debug:
                                print(
                                    f"{self.debug_logging_prefix} Final usage from API: {final_usage_stats}"
                                )

                            prompt_tok = final_usage_stats.get("prompt_tokens")
                            completion_tok = final_usage_stats.get("completion_tokens")
                            reasoning_tok = final_usage_stats.get(
                            ...
                            if override_occurred and self.debug:
                                print(
                                    f"{self.debug_logging_prefix} Using API token counts: Input={final_input_tokens_val}, Generated={final_generated_tokens_val}, Reasoning={final_reasoning_tokens_val}"
                                )
                        elif self.debug:
                            print(
                                f"{self.debug_logging_prefix} Using calculated tokens: Input={final_input_tokens_val}, Generated={final_generated_tokens_val}, Reasoning={final_reasoning_tokens_val}"
                            )

                        output_tokens_for_cost = final_generated_tokens_val
                        if not override_occurred:
                            output_tokens_for_cost = (
                                final_generated_tokens_val + final_reasoning_tokens_val
                            )
                        ...
                        cost_tracking_manager.calculate_costs_update_status_and_persist(
                            ...
                            status=(
                                ...
                                else "Error" if stream_error_occurred else "Stopped"
                            ),
                            persist_usage=True,
                            context_messages_count=len(body["messages"]),
                            web_search_requests_count=web_search_requests_count,
                        ...
                        if self.debug:
                            print(
                                f"{self.debug_logging_prefix} Stream completed. Final tokens - Input: {final_input_tokens_val}, Output: {output_tokens_for_cost}, Reasoning: {final_reasoning_tokens_val}"
                            )
                ...
                if self.debug:
                    print(
                        f"{self.debug_logging_prefix} LiteLLM making batch request..."
                    )
                ...
                if self.debug:
                    ...
                    print(
                        f"{self.debug_logging_prefix} Batch response: {response_summary}"
                    )
                ...
                if self.debug:
                    ...
                    print(
                        f"{self.debug_logging_prefix} Usage: input={usage_info.get('prompt_tokens', 'N/A')}, output={usage_info.get('completion_tokens', 'N/A')}"
                    )
                ...
                completion_tokens_details = api_usage.get(
                    "completion_tokens_details", {}
                )
                if (
                    completion_tokens_details
                    and completion_tokens_details.get("reasoning_tokens") is not None
                ):
                    api_reasoning_tokens = completion_tokens_details.get(
                        "reasoning_tokens", 0
                    )
                    if self.debug:
                        print(
                            f"{self.debug_logging_prefix} API reasoning tokens: {api_reasoning_tokens}"
                        )
                else:
                    ...
                    if (
                        ...
                        thinking_blocks_text = ""
                        thinking_blocks = choice.get("thinking_blocks") or message.get(
                            "thinking_blocks"
                        )
                        if thinking_blocks:
                            for tb in thinking_blocks:
                    ...
                    api_reasoning_tokens = cost_tracking_manager.count_tokens(
                        api_reasoning_content
                    )
                    if self.debug:
                        print(
                            f"{self.debug_logging_prefix} Calculated reasoning tokens: {api_reasoning_tokens}"
                        )
                ...
                if (
                    generate_thinking_block_flag
                    and api_reasoning_content
                    and response_json.get("choices")
                    and len(response_json["choices"]) > 0
                ):
                    main_content = response_json["choices"][0]["message"].get(
                        "content", ""
                    )
                    response_json["choices"][0]["message"][
                        "content"
                    ] = f"<think>{api_reasoning_content}</think>\n{main_content}"
                    if self.debug:
                        print(
                            f"{self.debug_logging_prefix} Injected thinking block in batch response"
                        )
            ...
            if (
                ...
                missing_key_provider = (
                    "the required provider (e.g., ANTHROPIC_API_KEY, OPENAI_API_KEY)"
                )
                ...
                "APIConnectionError" in error_message
                ...
                line_number = tb.tb_lineno
                user_message = (
                    f"LiteLLM Pipe error on line {line_number}: {error_message}"
                )
...
class Pipe:
    class Valves(BaseModel):
        DEBUG: bool = Field(
            default=False, description="Enable debug logging for LiteLLM Pipe module."
        )
