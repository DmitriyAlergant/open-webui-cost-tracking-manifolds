...
class UsagePersistenceManager:
    ...
    def _get_existing_columns(self, db, table_name, is_sqlite):
    ...
    def _create_table_if_not_exists(self, db, table_name, columns, is_sqlite):
        ...
        for col_name, col_def in columns:
            if "{auto_increment}" in col_def:
                ...
                col_def = col_def.format(auto_increment=auto_increment)
            formatted_columns.append(f"{col_name} {col_def}")
    ...
    def _add_missing_columns(self, db, table_name, missing_columns, is_sqlite):
        ...
        if is_sqlite:
            ...
            for col_name, col_def in missing_columns:
                # Remove auto_increment for ALTER TABLE (not applicable to new columns)
                if "{auto_increment}" in col_def:
                    continue  # Skip auto_increment columns in ALTER
                alter_sql = f"ALTER TABLE {table_name} ADD COLUMN {col_name} {col_def}"
                db.execute(text(alter_sql))
        else:
            # PostgreSQL can add multiple columns in one statement
            column_defs = []
            for col_name, col_def in missing_columns:
                if "{auto_increment}" in col_def:
                    continue  # Skip auto_increment columns in ALTER
                column_defs.append(f"ADD COLUMN IF NOT EXISTS {col_name} {col_def}")

            if column_defs:
                alter_sql = f"ALTER TABLE {table_name} {', '.join(column_defs)}"
                db.execute(text(alter_sql))
    ...
    def _init_db(self):
        ...
        required_schemas = self._required_table_schemas()
            ...
            with get_db() as db:
                for table_name, required_columns in required_schemas.items():
                    ...
                    self._create_table_if_not_exists(
                        db, table_name, required_columns, is_sqlite
                    )

                    existing_columns = self._get_existing_columns(
                        db, table_name, is_sqlite
                    )
                    ...
                    if missing_columns:
                        print(
                            f"{Config.INFO_PREFIX} Adding missing columns to {table_name}: {[col[0] for col in missing_columns]}"
                        )
                        self._add_missing_columns(
                            db, table_name, missing_columns, is_sqlite
                        )
                ...
                index_queries = [
                    ...
                    "CREATE INDEX IF NOT EXISTS idx_chat_id ON usage_costs_by_chat(chat_id)",
                ]
    ...
    async def log_usage_fact(
        ...
        insert_sql = """
            INSERT INTO usage_costs (
                       user_id, user_email,  model,  task,  metadata, timestamp,  input_tokens,  output_tokens,  reasoning_tokens,  total_cost,  display_cost,  cost_currency,  model_used_by_cost_calculation, web_search_requests, provider, last_updated_timestamp, last_updated_status
            ) VALUES (:user_id, :user_email, :model, :task, :metadata, :timestamp, :input_tokens, :output_tokens, :reasoning_tokens, :total_cost, :display_cost, :cost_currency, :model_used_by_cost_calculation, :web_search_requests, :provider, :last_updated_timestamp, :last_updated_status)
        """
        ...
        with get_db() as db:
            try:
                ...
                if result:
                    ...
                    display_cost = (
                        Decimal(str(result[1]))
                        if result[1] is not None
                        else Decimal(str(result[0]))
                    )
                ...
                print(
                    f"{Config.INFO_PREFIX} Database error in get_chat_total_cost: {e}"
                )
                raise
    ...
    async def update_chat_total_cost(
        ...
        with get_db() as db:
            try:
                ...
                if self.DEBUG:
                    print(
                        f"{Config.DEBUG_PREFIX} Updated chat {chat_id} total cost with additional {additional_cost} (display: {additional_display_cost}) {currency}"
                    )

            except Exception as e:
                print(
                    f"{Config.INFO_PREFIX} Database error in update_chat_total_cost: {e}"
                )
                raise
...
class ModelCostManager:
    ...
    def _find_best_match(self, query: str, strip_prefix: bool = False) -> str:
        ...
        print(
            f"{Config.INFO_PREFIX} _find_best_match: normalized_query: {normalized_query}, best_match: {best_match}"
        )
...
class CostCalculationManager:
    ...
    def calculate_costs(
        self, input_tokens: int, output_tokens: int, web_search_requests_count: int = 0
        ...
        if self.DEBUG:
            print(
                f"{Config.DEBUG_PREFIX} CostCalculationManager.calculate_costs called with: input_tokens={input_tokens}, output_tokens={output_tokens}, web_search_requests_count={web_search_requests_count}"
            )
            print(
                f"{Config.DEBUG_PREFIX} Model pricing data for {self.model_used_by_cost_calculation}: {self.model_pricing_data}"
            )
        ...
        input_display_cost_per_token = Decimal(
            self.model_pricing_data.get(
                "input_display_cost_per_token",
                self.model_pricing_data.get("input_cost_per_token", 0),
            )
        ) / Decimal(self.model_pricing_data.get("token_units", 1))
        ...
        output_display_cost_per_token = Decimal(
            self.model_pricing_data.get(
                "output_display_cost_per_token",
                self.model_pricing_data.get("output_cost_per_token", 0),
            )
        ) / Decimal(self.model_pricing_data.get("token_units", 1))

        # Calculate cost for web searches
        web_search_cost_per_request = Decimal(
            self.model_pricing_data.get("web_search_request_cost", 0)
        )
        ...
        web_search_display_cost_per_request = Decimal(
            self.model_pricing_data.get(
                "web_search_request_display_cost",
                self.model_pricing_data.get("web_search_request_cost", 0),
            )  # Fallback to real cost
        )
        total_web_search_display_cost = (
            web_search_requests_count * web_search_display_cost_per_request
        )
        ...
        total_cost = Decimal(float(compensation)) * (
            input_cost + output_cost + total_web_search_cost
        )
        ...
        input_display_cost = (
            input_tokens * input_display_cost_per_token if input_tokens else 0
        )
        output_display_cost = (
            output_tokens * output_display_cost_per_token if output_tokens else 0
        )
        total_display_cost = Decimal(float(compensation)) * (
            input_display_cost + output_display_cost + total_web_search_display_cost
        )
...
class CostTrackingManager:
    ...
    def __init__(
        ...
        self.cost_calculation_manager = CostCalculationManager(
            model=model, model_cost_manager=self.model_cost_manager
        )
    ...
    async def _load_chat_cost(self):
        ...
        self.chat_total_cost, self.chat_display_cost, self.chat_currency = (
            await self.usage_persistence_manager.get_chat_total_cost(self.chat_id)
        )
        self._chat_cost_loaded = True

        if self.DEBUG:
            print(
                f"{Config.DEBUG_PREFIX} Loaded chat {self.chat_id} total cost: {self.chat_total_cost} (display: {self.chat_display_cost}) {self.chat_currency}"
            )
    ...
    async def update_status_message(
        ...
        if current_display_cost is not None and cost_currency is not None:
            ...
            if (
                self.chat_id != "local"
                and input_tokens >= 20000
                and context_messages_count >= 20
            ):
        ...
        if generated_tokens:
            if reasoning_tokens:
                token_parts.append(
                    f"{generated_tokens} output tokens (incl. {reasoning_tokens} reasoning)"
                )
            else:
        ...
        status_parts = [
            (
                ...
                if (start_time and status in ("Completed", "Stopped"))
                else ""
            ),
        ...
        if self.DEBUG:
            print(
                f"{Config.DEBUG_PREFIX} status string update (emitted: {should_emit}): {status_message}"
            )
    ...
    def calculate_costs_update_status_and_persist(
        ...
        (
            ...
            total_display_cost,
            cost_currency,
            model_used_by_cost_calculation,
...
class Pipe:
    class Valves(BaseModel):
        DEBUG: bool = Field(
            default=False, description="Enable debug logging for usage tracking."
        )
