    ...
    def _create_table_if_not_exists(self, db, table_name, columns, is_sqlite):
        ...
        for col_name, col_def in columns:
            if '{auto_increment}' in col_def:
                auto_increment = "AUTOINCREMENT" if is_sqlite else "GENERATED BY DEFAULT AS IDENTITY"
        ...
        try:
            with get_db() as db:
                for table_name, required_columns in required_schemas.items():
                    ...
                    self._create_table_if_not_exists(db, table_name, required_columns, is_sqlite)
                    
                    existing_columns = self._get_existing_columns(db, table_name, is_sqlite)
                    ...
                    if missing_columns:
                        print(f"{Config.INFO_PREFIX} Adding missing columns to {table_name}: {[col[0] for col in missing_columns]}")
                        self._add_missing_columns(db, table_name, missing_columns, is_sqlite)
